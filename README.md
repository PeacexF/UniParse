...
- [Naming what the engine cannot](#naming-what-the-engine-cannot)
...
## Naming what the engine cannot

On a CSS-in-JS site the engine finds every column and can name only some of them: a real
timestamp lives in `span.sc-g7h8i9`, and no amount of vocabulary will guess that. `uparse
generate` is the optional escape from writing that config by hand.
...
```
  4 fields accepted (openai_compatible:gemini-3-flash)
    title            100%  the heading of each card
    price            100%  the only currency-shaped column
    stock            100%  'In stock' / 'Out of stock'
    published        100%  relative timestamps
    c6               ignored as interface text

wrote job.jsonc — review it, then uparse job.jsonc
```

**The model never extracts anything.** It is sent the same report `inspect` prints — 2–4 KB
against a 50–600 KB page — in which every column already has a selector the engine
validated across every record, and a number (`c1`, `c3`, …). It answers with ids. It cannot
express a selector, so it cannot invent one.

Then every id it cites is re-run against the real page. A field that does not resolve in
at least `min_coverage` of records is dropped, and the generated file says so:

```jsonc
// Generated by `uparse generate` on 2026-09-10.
// Source: https://example.com/products
// Engine: strategy dom, 20 records.
// Asked for: name, price, stock
// Dropped 'rating': resolves in 35% of records.
// Ignored c6: interface text, not data — e.g. 'Add to basket'.
// Review before running.
{
  "extraction": {
    "fields": {
      "title": "auto",                              // the engine already finds this
      "published": { "selector": "span.sc-g7h8i9" }  // 100% of records, e.g. '7 hrs ago'
    }
  }
}
```

A page is one call, the answer is cached by a hash of the report, and the file it writes
carries no `assist` block — it is a plain job that runs deterministically forever.

Providers, in the order most people will want them:

| provider | reaches | needs |
|---|---|---|
| `openai_compatible` | Gemini's free tier, Ollama, LM Studio, OpenRouter, Groq | `base_url` + a key env var |
| `command` | opencode, `llm`, `ollama run` — anything with a terminal | whatever that tool already has |
| `anthropic` | the Anthropic API | `pip install 'uniparse[llm]'` |
| `null` (default) | no model; `generate` writes what the engine inferred and says so | nothing |

Two things worth knowing. A page carries text an attacker wrote, and it goes into the
prompt — what contains that is not a line in the prompt but the shape of the thing: the
answer is a constrained set of ids, every one is validated against the DOM, and the result
is a file you read before anything runs. And none of this is ever required: `uparse scrape`
does not import the package, and with no key, no network and no model it behaves exactly
as it does today.
